...
def play_audio_bytes(audio_bytes, start_event, sync=True):
    ...
    Parameters:
      ...
      - start_event: threading.Event to wait for before starting playback, or None.
    ...
    try:
        ...
        # Only wait for the start_event if it's not None
...
def play_audio_from_memory(audio_data, start_event, sync=False):
    ...
    try:
        ...
        # Check if these are valid WAV bytes
        if not is_valid_wav(audio_data):
            print("Converting non-WAV audio bytes to valid WAV format")
            # Try to convert the raw bytes to WAV format
            try:
                # Assume 16-bit PCM mono audio at 22050Hz
                audio_file = convert_raw_to_valid_wav(audio_data)
                pygame.mixer.music.load(audio_file)
...
def is_valid_wav(audio_bytes):
    """Check if bytes represent a valid WAV file by checking header markers."""
    try:
        return (len(audio_bytes) > 44 and 
                audio_bytes[:4] == b'RIFF' and 
                audio_bytes[8:12] == b'WAVE' and
                audio_bytes[12:16] == b'fmt ')
    except:
        return False


def convert_raw_to_valid_wav(audio_bytes, sample_rate=22050, channels=1, sample_width=2):
    """
    Convert raw PCM bytes to a valid WAV file in memory.
