...
def merge_animation_data_into_facial_data(facial_data, animation_data, dimensions, alpha=0.6, blend_frame_count=32):
    # Safety check for facial_data
    if len(facial_data) == 0:
        return facial_data
        
    animation_data = adjust_animation_data_length(facial_data, animation_data)
    
    # Adjust blend_frame_count based on data length
    num_frames = len(facial_data)
    if num_frames < blend_frame_count:
        # If we have very few frames, reduce the blend_frame_count
        blend_frame_count = max(1, num_frames // 4)
